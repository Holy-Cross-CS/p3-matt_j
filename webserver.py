...
class Connection:
    def __init__(self, connected_socket, addr):
        ...
        self.keep_connect = True            # used for keeping the connection alive
...
def handle_one_http_request(conn):
    ...
    return req.headers # needed for Connection: keep-alive
...
def handle_http_get_hello(req, conn):
    ...
    if "?" in req.path:
        holder = req.path.split("?")
        variables = holder[1].split("&")
        varOne = variables[0].split("=")
        varTwo = variables[1].split("=")
        name = urllib.parse.unquote(varOne[1])
        favcolor = urllib.parse.unquote(varTwo[1])
    else:
        name = "hello"
        favcolor = "coral"
        x = get_cookies(req.headers)
        if x != None:
            name = get_cookie(req.headers, "name")
            print ("cookie name is " + name)
            favcolor = get_cookie(req.headers, "color")
            print ("cookie color is "+ favcolor)
        print(x)
    if favcolor not in [ "coral", "crimson", "navy", "purple", "tomato", "teal", "wheat" ]:
        favcolor = "weird"
    msg = f'<html><head>Welcome, {name}!<br>'
    msg += 'The current date is %s.\n' % (time.strftime("%a, %d %b %Y %H:%M:%S %Z"))
    msg += '</head>'
    msg += '<body>'
    msg += f'<p>This page has been visited {counter} time(s) before.'
    ...
    msg += '<p>  <a href="/hello"> This page</a>'
    msg += '<p>  <a href="/status"> Status and statistics</a>'
    msg += '<p>  <a href = "/quote"> A random quote</a>'
    msg += '<p>  <a href="/welcome.html"> A welcome page</a>' 
    msg += '<p>  <a href ="/index.html"> Some fancy web page</a><br>'
    msg += '<form method="GET" action="/hello">'
    msg += '<label for="inputName">Enter your name:</label><br>'
    msg += '<input type="text" id="inputName" name="inputName"><br>'
    msg += '<label for="inputColor">Enter your favorite color:</label><br>'
    msg += '<input type="text" id="inputColor" name="inputColor">'
    msg += '<input type="submit" value="Greet Me!">'
    msg += '</form>'
    # Need to figure out how to store the html info into the variables
    #msg += "<p>Just kidding, this page isn't yet interactive. Sorry."
    msg += "</body></html>"
    resp = Response("200 OK", "text/html", msg)
    resp.cookies = ["name="+name, "color="+favcolor]
    counter+=1
    return resp
...
def handle_http_get_file(url_path):
    ...
    try:
        with open(file_path, "rb") as f: # "rb" mode means read "raw bytes"
            data = f.read()
        # check for the mime type of the given file for formatting
...
def handle_http_connection(conn):
    ...
    try:
        ...
        start = time.time()
        # keep the connection open for 5 requests, as long as the connection wants to keep-alive
        count = 0
        while (conn.keep_connect == True):
            log("counter is " + str(count))
            if (count >= 5):
                conn.keep_connect = False
            if "keep-alive" in (get_header_value(handle_one_http_request(conn), "Connection")):
                count +=1 
            else:
                break 
        end = time.time()
